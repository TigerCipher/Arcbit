...
static Vec2 GetRefSize(const FramePacket& packet)
{
...
static void RegisterLifetimeSystem(World& world)
{
    world.RegisterSystem("Lifetime", [](Scene& scene, f32 dt) {
        std::vector<Entity> toDestroy;
        scene.GetWorld().Query<Lifetime>()
            .Without<Disabled>()
            .ForEach([&](Entity e, Lifetime& lt) {
                lt.Remaining -= dt;
...
static void RegisterFreeMovementSystem(World& world)
{
...
static void RegisterCameraFollowSystem(World& world)
{
    world.RegisterSystem("CameraFollow", [](Scene& scene, f32 dt) {
        scene.GetWorld().Query<const Transform2D, const CameraTarget>()
            .Without<Disabled>()
            .ForEach([&](const Transform2D& t, const CameraTarget& ct) {
...
static void RegisterSpriteRenderSystem(World& world)
{
    world.RegisterRenderSystem("SpriteRender", [](Scene& scene, FramePacket& packet) {
        ...
        scene.GetWorld().Query<const Transform2D, const SpriteRenderer>()
            .Without<Disabled>()
            .ForEach([&](Entity e, const Transform2D& t, const SpriteRenderer& sr) {
                Vec2 pos = t.Position;
                ...
                if (!cam.IsVisible(pos, t.Scale, ref)) return;
...
static void RegisterLightRenderSystem(World& world)
{
    world.RegisterRenderSystem("LightRender", [](Scene& scene, FramePacket& packet) {
        const Camera2D& cam = scene.GetCamera();
        const Vec2      ref = GetRefSize(packet);

        scene.GetWorld().Query<const Transform2D, const LightEmitter>()
            .Without<Disabled>()
            .ForEach([&](const Transform2D& t, const LightEmitter& le) {
                if (!cam.IsLightVisible(t.Position, le.Radius, ref)) return;
